...
class ContrastMemory(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.momentum = args.nce_m  # 非参数动量更新？
        self.kl = KLDiv(args.kd_T)

        stdv = 1. / math.sqrt(self.feat_dim / 3)
        for i in range(args.num_exps):  # 每一个专家网络都有一个Memory Bank，命名为memory_0、memory_1
            self.register_buffer('memory_' + str(i), torch.rand(args.n_data, args.feat_dim).mul_(2 * stdv).add_(-stdv))  # Memory Bank维度(1281167, 128)

    def forward(self, embeddings, pos_idx, neg_idx):
        batchSize = embeddings[0].size(0)
        idx = torch.cat([pos_idx.cuda(), neg_idx.cuda()], dim=1)  # anchor、正负样本的索引合起来
        K = self.args.pos_k + self.args.neg_k + 1  # anchor样本+正样本数量+负样本数量

        inter_logits = []
        soft_icl_loss = 0.
        for i in range(self.number_net):
            for j in range(i + 1, self.number_net):
                neg_rep = torch.index_select(getattr(self, 'memory_' + str(i)), 0, idx.view(-1)).detach()  # 从memory bank中选出anchor样本+正样本+负样本的embedding
                neg_rep = neg_rep.view(batchSize, K, self.feat_dim)  # 重新塑形状(bs, anchor+正负样本embedding数量, 128)
        ...
        mask[:, :self.args.pos_k + 1] = 1.  # anchor+正样本掩码
        icl_loss = 0.
...
class Sup_MCL_Loss(nn.Module):
    def __init__(self, args):
        ...
        self.embed_list = nn.ModuleList([])  # 投影头Projection Head
        self.args = args
        ...
        self.contrast = ContrastMemory(args)  # 初始化Memory Bank

    def forward(self, embeddings, pos_idx, neg_idx):

        for i in range(self.args.num_exps):
            embeddings[i] = self.embed_list[i](embeddings[i])  # 经过投影头映射成(128, 128)
        vcl_loss, soft_vcl_loss, icl_loss, soft_icl_loss = \
            self.contrast(embeddings, pos_idx, neg_idx)  # 以Memory Bank的方式进行对比学习
...
class Embed(nn.Module):
    ...
    def __init__(self, dim_in=1024, dim_out=128):
        ...
        self.proj_head = nn.Sequential(  # 两层线性层的投影头
