...
parser = argparse.ArgumentParser(description='Baseline CIFAR-LT Training')
parser.add_argument('--cfg_name', default='Baseline CIFAR100-LT-IF100', help='name of this configuration')
...
def main():
    ...
    if args.evaluate:
        validate(test_loader, model, criterion, 179, args, logger)
        return
    ...
    for epoch in range(0, args.epochs):  # args.start_epoch
        ...
        if epoch == args.cornerstone:
            logger.info('====> rt_classifier parameter begin update: {}'.format(model.rt_classifiers[0].weight.requires_grad))

        # train for one epoch
        train(train_loader if epoch >= args.cornerstone else train_loader, model, scaler,
              optimizer_crt if epoch >= args.cornerstone else optimizer_feat, epoch, args, logger, rank)
...
def train(train_loader, model, scaler, optimizer, epoch, args, logger, rank=0):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with autocast():  # amp混合精度库前向传播
            outputs, _ = model(images, (epoch >= args.cornerstone))  # 前向传播使用torch.float16(2字节)精度
...
def validate(val_loader, model, criterion, epoch, args, logger, rank=0):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # compute output
            outputs, _ = model(images, (epoch >= args.cornerstone))
